 ...
 *
 */
public class CutSearchST<Value> {
     ...
     */
    public CutSearchST(int capacity) { 
    ...
    public int rank(Integer key) {
        ...
        int lo = 0, hi = n-1, cut = 1; 
        if(lo <= hi)
        {
            double upper = Math.abs(key - min());
            int lower = max() - min();
            if(lower == 0) return 1;
            cut = (int) Math.floor(upper / lower); 
            if(cut >= n)
                cut = n - 1;
            else if(cut < 0)
                cut = 0;
        ...
        while (lo <= hi) { 
            int cmp = key.compareTo(keys[cut]);
            if      (cmp < 0) hi = cut - 1; 
            else if (cmp > 0) lo = cut + 1; 
            else return cut; 
            cut = lo + (hi - lo) / 2;
        } 
        return lo;
